...
METRIC_PROVENANCE: Dict[str, MetricProvenance] = {
    # TraCI reads are direct observations of the *simulation*, not field data.
    "average_speed_kmh": "SIMULATED",
    "average_waiting_seconds": "SIMULATED",
    "mean_completed_vehicle_waiting_seconds": "SIMULATED",
    "mean_active_vehicle_waiting_seconds": "SIMULATED",
    "max_vehicle_count": "SIMULATED",
    "average_travel_time_seconds": "SIMULATED",
    "mean_queue_length_meters": "SIMULATED",
    "stops_per_vehicle": "SIMULATED",
    "throughput_vehicles_per_hour": "SIMULATED",
    "co2_kg": "DERIVED",
    "nox_g": "DERIVED",
    "noise_db": "ESTIMATED",
    "pedestrian_delay_seconds": "ESTIMATED",
    "accessibility_score": "ESTIMATED",
    "departure_based_vehicle_delay": "SIMULATED",
    # SUMO emission model outputs collected through TraCI.
...
def _build_structured_metrics(metrics_dict: dict, is_fallback: bool = False) -> Dict[str, MetricValue]:
    ...
    return {
        ...
        "sumo_co2_kg": {
            ...
            "unit": "kg",
            "source": "sumo_traci_emission_model" if not is_fallback else src_tag,
            "provenance": "SIMULATED" if not is_fallback else prov_tag,
            "confidence": conf,
            "is_simulated": not is_fallback,
        },
...
def calculate_metrics(raw_result: RawSimulationResult) -> SimulationMetrics:
    ...
    result_dict: SimulationMetrics = {
        "simulation_id": raw_result.get("simulation_id"),
        "seed": raw_result.get("seed"),
        "movement_counts": raw_result.get("movement_counts", []),
